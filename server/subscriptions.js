...
io.sockets.on("connection", (socket) => {
  ...
  redisClient.hmget("managers", urlUuid, (error, manager) => {
    if (error) {
      logger.warn(
        `SOCKET: ON CONNECTION: attempting to get managers from redis- failed, socket_id: ${socket.id} emitting dbError now`
      );
      socket.emit("dbError");
      return;
    }
    ...
    if (manager) {
      logger.info(
        `SOCKET: ON CONNECTION: manager found, socket_id: ${socket.id}`
      );
      isIncomingStudent = false;
      let managerObj = JSON.parse(manager);
      roomToJoin = managerObj.roomId;
      if (
        socket.id != managerObj.sockedId &&
        managerObj.socketId in io.in(roomToJoin).connected
      ) {
        logger.info(
          `SOCKET: ON CONNECTION: manager already exists, socket_id: ${socket.id}, emitting attemptToConnectMultipleManagers now`
        );
        ...
        function terminateLecture() {
          ...
          redisClient.hmget("rooms", roomToJoin, (error, roomObj) => {
            ...
            logger.info(
              `SOCKET: Successfully deleted manager from redis, managerId: ${managerId}`
            );
            redisClient.hdel("rooms", roomToJoin, (error, success) => null);
            logger.info(
              `SOCKET: Successfully deleted room from redis, room_id: ${roomToJoin}`
            );
            const connected_sockets =
              io.sockets.adapter.rooms[roomToJoin].sockets;
        ...
        socket.on("updateBoards", (boardObj) => {
          redisClient.hmget("rooms", roomToJoin, (err, roomObj) => {
            ...
            socket.broadcast.to(roomToJoin).emit(
              "boards",
        ...
        io.of("/")
          ...
          .clients((error, clients) => {
            if (clients.length - 1 > 0) {
              ...
              logger.info(
                "SOCKET: start calling all students already in lecture"
              );
              socket.broadcast
                .to(roomToJoin)
                .emit("notifyPeerIdToManager", socket.id);
            }
          });
      }
    } else {
